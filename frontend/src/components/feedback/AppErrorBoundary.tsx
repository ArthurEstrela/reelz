...
export class AppErrorBoundary extends Component<AppErrorBoundaryProps, AppErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error('CineGiro UI error', error, errorInfo)
  }
    ...
    return (
      <main className="flex min-h-svh items-center justify-center bg-canvas p-6 text-center text-paper">
        <div className="max-w-sm border-y border-white/10 py-9">
          <svg className="mx-auto size-12 text-brand" viewBox="0 0 48 48" fill="none" aria-hidden="true">
          ...
          <p className="mt-2 text-sm leading-6 text-white/65">
            Seus dados continuam salvos. Recarregue o CineGiro para retomar de onde parou.
          </p>
          <button
            type="button"
            onClick={() => window.location.reload()}
            className="mt-6 w-full rounded-xl bg-brand px-5 py-3 font-bold text-white transition hover:bg-brand-bright"
          >
