...
export function PwaStatusPrompt() {
  ...
  const visible = updateAvailable || installPrompt !== null
    ...
    <AnimatePresence>
      {visible ? (
        <motion.aside
          ...
          className="fixed inset-x-4 bottom-24 z-[60] mx-auto flex max-w-md items-center gap-3 rounded-2xl border border-white/10 bg-[#17171e]/95 p-3 shadow-2xl backdrop-blur-xl"
        >
          <span
            className="grid size-11 shrink-0 place-items-center rounded-xl bg-reel text-lg font-black text-white"
            aria-hidden="true"
          >
            {updateAvailable ? '↻' : '↓'}
          </span>
          ...
          <button
            ...
            className="shrink-0 rounded-xl bg-white px-3 py-2.5 text-xs font-black text-black transition hover:bg-white/90 disabled:cursor-wait disabled:opacity-55"
          >
          ...
          <button
            ...
            className="grid size-8 shrink-0 place-items-center rounded-lg text-white/35 transition hover:bg-white/5 hover:text-white"
          >
